...
async function checkAllocations() {
    try {
        const response = await fetch(`${PTERODACTYL_PANEL_URL}/api/application/nodes/2/allocations?filter[server_id]=`, {
            headers: {
        ...
        console.log("Unassigned Allocations:", data?.data?.length);
    } catch(e) {
